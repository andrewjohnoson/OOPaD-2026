...
void LibraryManager::addBook(Library &lib) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        lib.addBook(new Book(id, title, author, year, copies));
    }
...
void LibraryManager::addEmployee(Library &lib) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        std::cout << std::endl;
        lib.addEmployee(new Employee(id, name, pos));
    }
...
void LibraryManager::createSubscription(Library &lib) {
    ...
    for (const Reader *reader : lib.getReaders().getAll()) {
    ...
    Reader* r = lib.findReader(readerId);
    ...
    if (lib.findSubscriptionByUser(readerId) == nullptr) {
        Subscription* s = new Subscription(subId, r);
...
void LibraryManager::assignEmployee(Library &lib) {
    ...
    for (const Employee *e : lib.getEmployees()) {
        std::cout << e->getId() << " ";
    }
    ...
    for (const Subscription *s : lib.getSubscriptions().getAll()) {
        std::cout << s->getId() << " ";
    }
...
void LibraryManager::addBookToSubscription(Library &lib) {
    ...
    for (const Book *b : lib.getBooks().getAll()) {
        std::cout << b->getId() << " ";
    }
    ...
    while (true) {
        ...
        } else if (b->getAvailableCopies() == 0) {
            std::cout << "Доступных книг больше нет." << std::endl;
        } else {
            break;
        }
    }
    ...
    if(!s->isEditable())
    {
        std::cout << "Абонемент обрабатывается сотрудником. Изменения невозможны" << std::endl;
        TerminalController::waitForClick();
        return;
    }
...
void LibraryManager::load(Library &lib) {
    ...
    while (std::getline(subscriptionsFile, line)) {
        ...
        std::string workerCountStr;
        getline(ss, workerCountStr);
        const int workerCount = std::stoi(workerCountStr);
        s->setWorkerCount(workerCount);
    ...
    while (std::getline(employeesFile, line)) {
        ...
        if (subId != -1) {
            Subscription *sub = lib.findSubscription(subId);
...
void LibraryManager::save(Library &lib) {
    ...
    for (const Reader *r : lib.getReaders().getAll()) {
    ...
    for (const Subscription *s : lib.getSubscriptions().getAll()) {
        ...
        for (const Book *book : s->getBooks()) {
            subscriptionsFile << book->getId() << ";";
        }
        subscriptionsFile << s->getWorkerCount() << std::endl;
    }
    subscriptionsFile.close();
